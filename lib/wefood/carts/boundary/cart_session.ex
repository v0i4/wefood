defmodule Wefood.Carts.Boundary.CartSession do
  @moduledoc """
  Cart Cache server handler
  """
  use GenServer
  alias Wefood.Carts.Core.HandleCarts, as: Cart
  ...
  def handle_cast({:create, cart_id}, name) do
    case find_cart(name, cart_id) do
      {:not_found, []} -> :ets.insert(name, {cart_id, Cart.create_carts(cart_id)})
  ...
  def handle_call({:inc, cart_id, product}, _from, name) do
    {:ok, cart} = find_cart(name, cart_id)
    cart = Cart.inc(cart, product)
    :ets.insert(name, {cart_id, cart})
    {:reply, cart, name}
  end

  def handle_call({:dec, cart_id, product}, _from, name) do
    {:ok, cart} = find_cart(name, cart_id)
    cart = Cart.dec(cart, product)
    :ets.insert(name, {cart_id, cart})
    {:reply, cart, name}
  end

  def handle_call({:remove, cart_id, product}, _from, name) do
    {:ok, cart} = find_cart(name, cart_id)
    cart = Cart.remove(cart, product)
    :ets.insert(name, {cart_id, cart})
    {:reply, cart, name}
  end
